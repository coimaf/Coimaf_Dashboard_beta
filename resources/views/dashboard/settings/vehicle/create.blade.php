            ...
            <x-allert />
                 ...
                 <div class="row g-3 m-4">
                    <div class="col-12 col-md-6">
                        <form action="{{ route('dashboard.settings.vehicle.store') }}" method="post">
                            @csrf
                            <label class="mb-3" for="name">Tipo Veicolo: </label>
                    ...
                    <div class="col-12 col-md-3 ps-3 ms-3 pt-3 bg-white border border-2">
                        ...
                        <form action="{{ route('dashboard.settings.vehicle.delete', ['vehicleType' => $vehicleType->id]) }}" method="post">
                            @csrf
                            @method('delete')
                            <label class="mb-3" for="name">Tipo Veicolo: </label>
                            <span class="badge bg-primary ms-3">{{$vehicleType->name}}</span>
                            <button class="btn" type="submit"><i class='bi bi-trash-fill text-danger fs-5'></i></button>
                        </form>
                        @endforeach
                        @else
                        <p class="fw-semibold text-center">Non ci sono Veicoli</p>
                        @endif
                    </div>
                </div>
